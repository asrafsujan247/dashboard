    ...
    <div className="table-toolbar">
      ...
      {isXs ? (
        <>
          ...
          <div
            className={clsx(
              "hide-scrollbar flex shrink-0 gap-2 overflow-x-auto pt-4 pb-1",
              isFullScreenEnabled ? "px-4 sm:px-5" : "px-(--margin-x)",
            )}
          >
            <Filters table={table} />
        ...
        <div
          className={clsx(
            "custom-scrollbar transition-content flex justify-between gap-4 overflow-x-auto pt-4 pb-1",
            isFullScreenEnabled ? "px-4 sm:px-5" : "px-(--margin-x)",
          )}
          style={
            {
...
function SearchInput({ table }: { table: Table<Course> }) {
  return (
    <Input
...
function Filters({ table }: { table: Table<Course> }) {
  const isFiltered = table.getState().columnFilters.length > 0;
  const toolbarFilters = table.getState()?.toolbarFilters;

  invariant(toolbarFilters, "toolbarFilters is required");
